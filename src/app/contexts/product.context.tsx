...
const ProductsContextProvider = ({
  ...
  const updateCartItemQuantity = (item: Product, quantity: number) => {
    if (quantity < 1) {
      setCart((prevItem) =>
        prevItem.filter((cartItem) => cartItem.id !== item.id)
      )
    } else {
      setCart((prevItem) => {
  ...
  return (
    <ProductContext.Provider
      value={{ cart, setCart, toggleCart, updateCartItemQuantity }}
    >
      {children}
    </ProductContext.Provider>
  )
...
export const useProductContext = () => {
  ...
  if (context === null)
    throw new Error('O Product context deve estar dentro do provider')
  return context
}
