    ...
    <>
      ...
      <Container className="grid grid-cols-1 overflow-auto auto-rows-[150px] md:grid-cols-2 min-[1060px]:grid-cols-3 gap-4 mt-4] h-[calc(100vh-(231px+64px))] py-4">
        {cart.map((item, index) => (
          <div
            key={item.id}
            className="bg-white grid grid-cols-3 shadow-md p-3 rounded-md"
          >
            ...
            <div className="col-span-2 flex flex-col justify-between">
              <TitleCard className="line-clamp-2 border-b-2 border-emerald-600 md:text-lg">
                {item.title}
              </TitleCard>
      ...
      <div className="border h-[25vh] fixed bottom-0 left-0 right-0 w-full bg-emerald-400">
        <Container>
          ...
          <h1 className="text-center my-4 font-bold text-2xl sm:text-3xl md:text-5xl">
