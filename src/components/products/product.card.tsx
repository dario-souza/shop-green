...
const ProductCard = ({ data, index }: ProductCardProps) => {
  ...
  return (
    <div className={`text-center border bg-white rounded-md p-2 overflow-hidden shadow-md ${isInCart && 'ring-4 ring-emerald-700'}`}>
      <Image
        priority={index < 8}
      ...
      />
      <TitleCard className='border-b-2 line-clamp-1 border-emerald-700'>{data.title}</TitleCard>
      <PriceCard>
