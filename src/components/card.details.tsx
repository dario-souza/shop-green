...
export const CardDetails = ({ data }: CardDetailsProps) => {
  ...
  return (
    <div className='md:grid md:grid-cols-2 gap-5 md:pl-4 items-center'>
      ...
      <div className='bg-slate-300 rounded-lg p-4'>
        ...
        <Button onClick={() => toggleCart(data)} className={`block mt-2 py-2 text-white md:text-3xl md:mt-4 text-xl font-bold w-full ${incart ? 'bg-red-500' : 'bg-green-500'} rounded-md`}>{incart ? 'Remover' : 'Adicionar'}</Button>
      </div>
    </div>
  )
}
